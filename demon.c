					...
					if ((destinationEntry->d_type) == DT_REG)
					{
						...
						if (strcmp(sourceEntry->d_name, destinationEntry->d_name) == 0)
						{
							...
							if (strcmp(modTimeSrc, modTimeDest) != 0)
							{
								...
								{
									currenTime();
									printf("Wystąpił problem podczas próby uzyskania pełnej ścieżki do pliku docelowego\n");
									syslog(LOG_ERR, "Wystąpił problem podczas próby uzyskania pełnej ścieżki do pliku docelowego\n");
									exit(EXIT_FAILURE);
								}
								...
								{
									currenTime();
									printf("Wystąpił problem podczas próby ustawienia czasu pliku docelowego\n");
									syslog(LOG_ERR, "Wystąpił problem podczas próby ustawienia czasu pliku docelowego\n");
									exit(EXIT_FAILURE);
								}
							}
						}
